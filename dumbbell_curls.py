...
class DumbbellCurlDetector:
    ...
    def process(self, image, landmarks):
        """
        Processes the image and landmarks to count dumbbell curls (both arms).
        """
        h, w, _ = image.shape
        ...
        if l_visible:
            ...
            if angle < 30 and self.stage_left == 'down':
                self.stage_left = "up"
                self.counter += 1
                print(f"Dumbbell Curl count: {self.counter} (Left)")
        ...
        if r_visible:
            ...
            if angle < 30 and self.stage_right == 'down':
                self.stage_right = "up"
                self.counter += 1
                print(f"Dumbbell Curl count: {self.counter} (Right)")
